...
class ProductFormTestCase(TestCase):
    ...
    def test_product_form_name_rejects_special_characters(self):
        """Special characters other than diacritics and basic punctuation
        are rejected in the name, since the generated slug can't represent
        them.
        """
        invalid_names = [
            'Prusa Core One+',
        ...
        for name in invalid_names:
            with self.subTest(name=name):
                form_data = {**self.valid_data, 'name': name}
                form = ProductForm(data=form_data)

                self.assertFalse(form.is_valid())
                self.assertIn('name', form.errors)

    def test_product_form_name_allows_diacritics_and_basic_punctuation(self):
        """Diacritics, digits, spaces, hyphens, commas, periods, and
        parentheses in the name are allowed.
        """
        form_data = {
            **self.valid_data,
            'name': 'Spectrum PLA – černá, matná (1 kg)',
        }
        form = ProductForm(data=form_data)
    ...
    def test_product_form_slug_autogenerated_from_name(self):
        """Blank slug is auto-generated from the product name."""
        form_data = {**self.valid_data, 'slug': ''}
        form = ProductForm(data=form_data)

        self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data['slug'], 'spectrum-pla-cerna')

    def test_product_form_slug_overwritten_from_name(self):
        """A manually entered slug is overwritten by one generated from the
        name.
        """
        form_data = {**self.valid_data, 'slug': 'nesouvisejici-slug'}
        form = ProductForm(data=form_data)

        self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data['slug'], 'spectrum-pla-cerna')

    def test_product_form_slug_strips_special_characters(self):
        """Diacritics and allowed punctuation (comma, period, parentheses)
        are removed from the generated slug, even though they're allowed in
        the name itself.
        """
        form_data = {
            **self.valid_data,
        ...
        self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data['slug'], 'spectrum-pla-cerna-matna-1-kg')

    def test_product_form_slug_uniqueness_appends_suffix(self):
        """A duplicate generated slug gets a numeric suffix to stay unique."""
        ...
        self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data['slug'], 'spectrum-pla-cerna-2')

    def test_product_form_slug_uniqueness_ignores_own_instance(self):
        """Editing a product keeps its own slug even though it would
        ...
        form = ProductForm(data=self.valid_data, instance=existing)
